...
THRESHOLD   = 0.0401   # Zone 1 upper -- 92.5% sensitivity floor
ZONE2_UPPER = 0.0693   # Zone 2 upper -- obs rate matches COSI-eligible (5.61%)
ZONE3_UPPER = 0.0772   # Zone 3 upper -- obs rate matches ambulatory (6.20%) -- hard cutoff
AUC       = 0.740
N_TRAIN   = 309_529
...
def do_reset():
    # Clear all session state keys so widgets revert to their default values.
    # Explicitly set widget keys to defaults to force Streamlit to re-render
    # number inputs, radios, and checkboxes at their initial values.
    for k in list(st.session_state.keys()):
        del st.session_state[k]
    # Reinitialise result/recalc flags
    st.session_state.result = None
    st.session_state.needs_recalc = False
    # Explicitly zero out all checkbox keys so they render unchecked
...
def score(inp):
    ...
    # wbc_missing: only flag if leukopenia is indicated AND value not entered.
    # Without this gate, every patient without leukopenia gets wbc=NaN which
    # triggers wbc_missing=1.0, incorrectly signalling unordered labs for all
    # healthy patients and pushing their risk above threshold.
    f["wbc_missing"]               = float(inp.get("has_leukopenia", False) and
                                           (inp["wbc_cat"] == "missing" or
                                            np.isnan(f.get("wbc", np.nan))))
...
def make_gauge(prob, threshold=THRESHOLD):
    """SVG semicircle gauge with three clinical zones (capped at 15%):
       Green  <= 4.01%  Zone 1 recommended ambulatory
       Amber  4.01-6.93% Zone 2 clinician discretion
       Orange 6.93-7.72% Zone 3 extended discretion / hard cutoff
    """
    MAX_DISP = 0.10   # cap at 10% -- keeps zones proportional
    W, H = 340, 220
    cx, cy, r_out, r_in = 170, 200, 150, 95
    ...
    # Four zones matching DCA figure
    zones = [
        (0,           THRESHOLD,   "#1a4731", "#22c55e"),   # green
        (THRESHOLD,   ZONE2_UPPER, "#3b2700", "#f59e0b"),   # amber
        (ZONE2_UPPER, ZONE3_UPPER, "#3b1a00", "#f97316"),   # orange
        (ZONE3_UPPER, MAX_DISP,    "#3b0a0a", "#ef4444"),   # red
    ]

    # Needle color matches zone
    if prob <= THRESHOLD:
        needle_color = "#22c55e"
    elif prob <= ZONE2_UPPER:
        needle_color = "#f59e0b"
    elif prob <= ZONE3_UPPER:
        needle_color = "#f97316"
    else:
        needle_color = "#ef4444"
    ...
    # Colored zones -- fill AND outer arc stroke in matching color
    for z0, z1, fill_bg, fill_arc in zones:
        ...
        # Inner arc highlight (replaces the outer rim that obscured labels/ticks)
        svg += f'<path d="M {xi0:.1f} {yi0:.1f} A {r_in} {r_in} 0 {large} 0 {xi1:.1f} {yi1:.1f}" fill="none" stroke="{fill_arc}" stroke-width="3" opacity="0.7"/>'

    # Zone boundary tick marks (inner only, no labels cluttering outer)
    for thr_z, color_z in [(THRESHOLD, "#22c55e"), (ZONE2_UPPER, "#f97316"), (ZONE3_UPPER, "#ef4444")]:
        if thr_z <= MAX_DISP:
    ...
    # Zone boundary percentage labels
    for thr_z, color_z, lbl in [
        (THRESHOLD,   "#22c55e", f"{100*THRESHOLD:.2f}%"),
        (ZONE2_UPPER, "#f97316", f"{100*ZONE2_UPPER:.2f}%"),
        (ZONE3_UPPER, "#ef4444", f"{100*ZONE3_UPPER:.2f}%"),
    ]:
        if thr_z <= MAX_DISP:
    ...
    if prob <= THRESHOLD:
        zone_str = "Zone 1 — Recommended"
    elif prob <= ZONE2_UPPER:
        zone_str = "Zone 2 — Discretion"
    elif prob <= ZONE3_UPPER:
        zone_str = "Zone 3 — Ext. Discretion"
    else:
        zone_str = "Inpatient Required"
    svg += f'<text x="{cx}" y="{cy-28}" fill="{needle_color}" font-size="28" font-weight="700" text-anchor="middle" font-family="Courier New">{pct_str}</text>'
...
with col_result:
    ...
    if res is None:
        ...
        # Verdict
        if cal_prob <= THRESHOLD and not active_flags:
            v_bg,v_bd,v_tc = "#0f2a1a","#166534","#4ade80"
            v_title = "Zone 1 — Recommended Ambulatory Candidate"
            v_body  = f"Predicted risk ({100*cal_prob:.1f}%) is below {100*THRESHOLD:.2f}%. Model sensitivity: 92.5%. Fewer than 2.5 adverse events per 100 patients cleared at this threshold."
        elif cal_prob <= THRESHOLD and active_flags:
            v_bg,v_bd,v_tc = "#0d1f18","#065f46","#34d399"
            v_title = "Zone 1 — Candidate with Active Flag"
            v_body  = f"Predicted risk is below {100*THRESHOLD:.2f}% despite high-risk flag(s). Specialist clearance recommended before proceeding."
        elif cal_prob <= ZONE2_UPPER and not active_flags:
            v_bg,v_bd,v_tc = "#2d1f00","#92400e","#fbbf24"
            v_title = "Zone 2 — Clinician Discretion"
            v_body  = f"Predicted risk ({100*cal_prob:.1f}%) is between {100*THRESHOLD:.2f}% and {100*ZONE2_UPPER:.2f}%. Patients at this level have observed event rates equivalent to COSI-eligible patients (5.61%). Surgeon judgment governs."
        elif cal_prob <= ZONE2_UPPER and active_flags:
            v_bg,v_bd,v_tc = "#2d1f00","#92400e","#fbbf24"
            v_title = "Zone 2 — Clinician Discretion with Active Flag"
            v_body  = f"Predicted risk is in the discretion zone and active flag(s) are present. Consider specialist clearance."
        elif cal_prob <= ZONE3_UPPER:
            v_bg,v_bd,v_tc = "#2d1000","#c2410c","#fb923c"
            v_title = "Zone 3 — Extended Discretion"
            v_body  = f"Predicted risk ({100*cal_prob:.1f}%) is between {100*ZONE2_UPPER:.2f}% and {100*ZONE3_UPPER:.2f}%. Patients at this level approach the observed event rate of current ambulatory patients (6.20%). Proceed with caution."
        else:
            v_bg,v_bd,v_tc = "#1f0a0e","#991b1b","#f87171"
            v_title = "Inpatient Setting Required"
            v_body  = f"Predicted risk ({100*cal_prob:.1f}%) exceeds {100*ZONE3_UPPER:.2f}%. Above this threshold, expansion candidates experience adverse event rates higher than current ambulatory patients. Inpatient monitoring warranted."
