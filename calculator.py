...
def make_gauge(prob, threshold=THRESHOLD):
    """SVG semicircle gauge with four clinical zones:
       Green  <= 4.01%  Zone 1 recommended ambulatory
       Yellow 4.01-6.93% Zone 2 clinician discretion
       Orange 6.93-7.72% Zone 3 extended discretion
       Red    > 7.72%   Inpatient required
    """
    MAX_DISP = 0.30
    W, H = 340, 195
    ...
    # Four clinical zones matching the DCA figure
    zones = [
        (0,           THRESHOLD,   "#1a4731", "#22c55e"),   # green
        (THRESHOLD,   ZONE2_UPPER, "#3b2700", "#f59e0b"),   # yellow/amber
    ...
    for thr_z, color_z in [(THRESHOLD, "#22c55e"), (ZONE2_UPPER, "#f97316"), (ZONE3_UPPER, "#ef4444")]:
        tx0z, ty0z = arc_pt(thr_z, r_out + 8)
        tx1z, ty1z = arc_pt(thr_z, r_in - 8)
        svg += f'<line x1="{tx0z:.1f}" y1="{ty0z:.1f}" x2="{tx1z:.1f}" y2="{ty1z:.1f}" stroke="{color_z}" stroke-width="1.5" stroke-dasharray="3,2" opacity="0.8"/>'

    # Zone boundary labels
    for thr_z, color_z, lbl in [
        (THRESHOLD,   "#22c55e", f"{100*THRESHOLD:.1f}%"),
        (ZONE2_UPPER, "#f97316", f"{100*ZONE2_UPPER:.1f}%"),
        (ZONE3_UPPER, "#ef4444", f"{100*ZONE3_UPPER:.1f}%"),
    ]:
        ttx, tty = arc_pt(thr_z, r_out + 26)
